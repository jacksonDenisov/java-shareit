...
class BookingControllerTest {
    ...
    @Test
    void getBadRequestWhenCreateNewBookingWithEmptyDto() throws Exception {
        when(bookingService.create(any(), anyLong())).thenReturn(bookingDtoToUser);
        mvc.perform(post("/bookings")
                        .content(mapper.writeValueAsString(new BookingDtoFromUser()))
                        .header("X-Sharer-User-Id", 1)
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
    ...
    @Test
    void findAllBookingsByBookerId() throws Exception {
        when(bookingService.findAllBookingsByBookerId(anyLong(), anyString(), any())).thenReturn(new ArrayList<>());
    ...
    @Test
    void getBadRequestWhenFindAllBookingsByBookerIdWithoutHeader() throws Exception {
        when(bookingService.findAllBookingsByBookerId(anyLong(), anyString(), any())).thenReturn(new ArrayList<>());
        mvc.perform(get("/bookings")
                        .content(mapper.writeValueAsString(bookingDtoFromUser))
                        .characterEncoding(StandardCharsets.UTF_8)
    ...
    @Test
    void findAllBookingsByItemOwner() throws Exception {
